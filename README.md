...
for one real Edge Function under `backend/functions/` that does the one
thing the browser's anon key genuinely can't: permanently deleting a
user's login (`permanently-erase-account`).
